...
Current version: **0.18.3**. Supported game build: **Starsector 0.98a-RC8**.
...
The public API is a roadmap item, not a published compatibility surface in `0.18.3`. Its intended
...
`1.0.14-spp13`. The fork is required for optimal AoTD performance and for the supported native
...
- [`docs/releases/0.18.3.md`](docs/releases/0.18.3.md) — current detailed release report.

StarsectorPrepatcher is distributed under the terms in [`LICENSE`](LICENSE).

## AoTD Scheduler Fork integration

Prepatcher 0.18.3 supports Scheduler Fork `1.0.14-spp13` and installs a clean wrapper around the
...
than `1.0.14-spp13`, or a declared mask other than `0xfff` is logged and rejected with no partial
compatibility mode.
...
The maintained Scheduler Fork `1.0.14-spp13` is required for optimal performance when AoTD Theory of
