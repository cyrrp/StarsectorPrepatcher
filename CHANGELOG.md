...
- Начиная с этого выпуска, к MIT License применяется Commons Clause License Condition v1.0:
  исходный код остаётся доступным для использования, изменения и распространения, но продажа
  самого StarsectorPrepatcher в определённом Commons Clause смысле не разрешается. Предыдущие
  выпуски сохраняют условия, на которых они были опубликованы.

### Исправлено

- Добавлена совместимость с Java 27 без Faster Rendering и с обеими его архитектурами: обычная
  цепочка Prepatcher, новый `fr.agent.jar` и старый pre-define `AppClassLoader`. Активность FR
  определяется по фактическим параметрам JVM, а заявленный небезопасный маршрут завершается до
  загрузки игровых классов.
- Для обычного Java 27 pipeline и старого FR pre-define bridge встроено согласованное исправление
  запрещённых обфускатором имён вроде `while.new`; Java 27 больше не получает такие constant-pool
  entries в `defineClass`.
- Trade и market-overview UI-патчи принимают как исходные имена полей рынка, так и их точные
  варианты после Java-27/FR repair; поздний Cargo-патч больше не теряется из-за отката общего
  pipeline.
- В Local Resources tooltip добавлен точный stack-map `F_SAME`, устраняющий `VerifyError` Java 27
  без глобального пересчёта frames.
- JDK-internal ASM заменён официальным ASM 9.10.1, изолированным внутри agent JAR; сборка остаётся
...
Подробности и состав проверок: [отчёт о выпуске 0.18.4](docs/releases/0.18.4.md).
...
- Точный runtime-контракт переведён на Scheduler Fork `1.0.14-spp12`.
- Выпуск собран на проверенной кодовой базе Prepatcher `0.18.0`; откаченная
  в AoTD схема контрольных точек/доказательств сделок в этот agent не входит.
- Сохранены bridge schema V10, production mask `0xbff`, declared mask `0xfff` и
  restore-completion hook, необходимый для безопасного восстановления supply/demand.

Подробности и состав проверок: [отчёт о выпуске 0.18.2](docs/releases/0.18.2.md).

## [0.18.1] - 2026-08-10

### Изменено

- Точный runtime-контракт был переведён на Scheduler Fork `1.0.14-spp11` для
  поддержки новой схемы восстановления economy tasks и proof-gated trade publication.
- Diagnostic discovery перестал считать JAR-копии из `.build` и `releases` отдельными
  установленными fork-кандидатами.
- Выпуск отозван вместе с AoTD `spp11`: новая economy-схема вызвала сильную
  регрессию производительности. Актуальная пара — `0.18.2` + AoTD `spp12`.

Историческая сводка: [отчёт о выпуске 0.18.1](docs/releases/0.18.1.md).
...
- Точный контракт поддерживаемого AoTD Scheduler Fork обновлён до `1.0.14-spp8`. Новый fork
  восстанавливает отсутствовавший в JAR Domain UI и требует AshLib 2.2.3; schema V9 и capability
  masks `0x3ff`/`0x7ff` не изменились.
- Версия agent/bootstrap, launch scripts, профили, metadata и документация синхронизированы на
  `0.17.1`; `spp7`, будущие и частичные контракты по-прежнему отклоняются целиком.

Подробности и состав проверок: [отчёт о выпуске 0.17.1](docs/releases/0.17.1.md).
