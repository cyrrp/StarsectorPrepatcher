...
Статус: план работ после выпуска `0.18.2`. Economy-restore coordination с AoTD и атомарный UI market-mutation refresh для policy/trade/free-port и exact vanilla industry branches добавлены; Persistent economy snapshots, ordered commodity active
...
выпуска этим документом не задаётся.
