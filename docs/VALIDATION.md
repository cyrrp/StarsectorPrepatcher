...
причины/остаточного риска в отчёте соответствующего выпуска из [`releases/`](releases/0.17.0.md).
...
- bridge schema V9 / required capability mask `0x3ff` and optional full mask `0x7ff`;
- exact-current registration matrix: safe profile still returns exactly `0x3ff`, enabled optional
  mutation refresh returns `0x7ff`, while spp4-spp6, future identifiers, wrong declared mask and
  missing callbacks return `0` with a reason-specific rejection status;
- bridge-transform negatives for V8, future V10 and malformed current V9, proving that no legacy
  schema or partial current shape is rewritten;
- exact detached-Cargo/LOOT call-site guards, final vanilla `Economy.tripleStep/getEconomy` contract, exact-vanilla skip, spp7 explicit dispatch, subclass/rejection/error fallback, idempotence and changed-body fail-closed fixtures;
...
- bridge capability masks `0x3ff` required (including explicit UI dispatch) and `0x7ff` with the
  optional market-mutation refresh;
