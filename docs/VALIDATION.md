...
причины/остаточного риска в отчёте соответствующего выпуска из [`releases/`](releases/0.18.1.md).
...
- exact-current registration matrix: safe profile still returns exactly `0xbff`, enabled optional
  mutation refresh returns `0xfff`, while spp4-spp10, future identifiers, wrong declared mask and
...
- diagnostic marker scan with the installed fork under `jars/` and duplicate marker JARs under
  `.build/audit/` and `releases/staging/`; only the installed JAR may remain a candidate and removing
  it must produce `NOT_FOUND`, never `AMBIGUOUS`;
- exact detached-Cargo/LOOT call-site guards, final vanilla `Economy.tripleStep/getEconomy` contract, exact-vanilla skip, spp11 explicit dispatch, subclass/rejection/error fallback, idempotence and changed-body fail-closed fixtures;
...
- actual-fork XStream 1.4.10 migration of a fixed spp9 XML fixture and spp11 round-trip proving that
...
- actual-fork XStream 1.4.10 task-graph sanitization proving that a live save detaches the
  in-process list only for cleanup and never restores it; both successful and failed save outcomes
  release the stale graph and preserve only a semantic restart checkpoint, while legacy spp10 task
  graphs are dropped under a load guard before spp11 resumes without changing calendar cadence;
- MAIN checkpoint coverage for `FULL` before the first market commit, exact ordered
  `PRICE_REMAINING` stable IDs after partial progress, `LISTENERS_ONLY` after all market commits and
  `DROP` for legacy unknown progress; resumed MAIN must rebuild transient global commodity data from
  the complete market scope, must not repeat already-attempted price/stockpile/month commits and
  must not lose the unattempted suffix;
- same-process success/failure and nested-save harnesses proving reverse Finish/Post/Main cleanup,
  future cancellation, ticket-debt restoration, no stale DTO/boundary reuse, exactly one outer
  cleanup, and a `WAITING`/no-checkpoint discard with no invented restart;
- real-fork structural proof that an `EMPTY`/unknown registry bulk-defers scheduler-owned commits
  and that the dedicated materialized-input-generation fast path also requires the exact
  pre-immigration market-size checkpoint, while trade/accessibility/faction-only work leaves that
  token unchanged; vanilla size growth with unchanged trade values takes one coherent live fallback,
  queues one coalesced materialized repair, returns to the fast path after scheduler materialization,
  and a registry clear/rebuild cannot reuse an old token;
- real-fork multi-frame proof capture with a scalar/token change between frames: only the stale
  market is recaptured, diagnostics describe the final cut, the complete proof set is validated
  under the registry lock before manager publication, and a residual `STALE_INPUT` preserves dirty
  work; an injected manager publication failure rolls back the complete cut and leaves the post task
  done, including the mapping-without-snapshot rollback case;
- structural/runtime proof that coalescing an already-existing materialized repair retains
  VALUE/PRICE/STOCK work without advancing the dedicated materialized-input token again; pure
  price/stockpile/accessibility/trade debt returns from `AoTDUpdateMarketAgainTask` before any
  industry traversal, while month-end explicitly invalidates VALUE/DERIVED/PRICE/STOCK before the
  required materialized pass;
- recommended, non-automated real-campaign save/load acceptance smoke with population industry
  first and a later unavailable industry; it must confirm structural-only restore, one coalesced
  scheduler revision, `NOT_READY`
  materialized-input-generation invariants, visible calculation-script failures, and
  successful/failed-save recovery;
...
- pre-epoch serialized task safe drop, semantic phase checkpoint and lazy post-bind restart;
- successful-save and failed-save semantic reconstruction after the detached in-process graph is
  released before the worker/save barrier; no live task identity, captured ticket, worker future,
  prepared POST cut or FINISH boundary may be reused;
- `EMPTY`-registry bulk deferral until `onGameLoad` publishes the live epoch; no claim or committed
  revision may be created while restore coordination is still pending.
