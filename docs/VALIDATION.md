...
причины/остаточного риска в отчёте соответствующего выпуска из [`releases/`](releases/0.17.2.md).
...
- exact-current registration matrix: safe profile still returns exactly `0x3ff`, enabled optional
  mutation refresh returns `0x7ff`, while spp4-spp8, future identifiers, wrong declared mask and
...
- exact detached-Cargo/LOOT call-site guards, final vanilla `Economy.tripleStep/getEconomy` contract, exact-vanilla skip, spp9 explicit dispatch, subclass/rejection/error fallback, idempotence and changed-body fail-closed fixtures;
